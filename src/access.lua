...
function  handle_callback( conf, callback_url )
    ...
    if code then
		...
		if type(ngx.header["Set-Cookie"]) == "table" then
			...
			ngx.log(ngx.WARN, "set test cookie 1")
			ngx.header["Set-Cookie"] = { "MyTestCookie1=" .. "testValue1" .. ";Path=/;Expires=" .. ngx.cookie_time(ngx.time() + 1800) .. ";Max-Age=1800;HttpOnly", ngx.header["Set-Cookie"] }
        end
